import React, { useState } from "react";
import Navbar from "../navbar/Navbar";
import { BiBell } from "react-icons/bi";
import { FaRegMoneyBillAlt } from "react-icons/fa";
import { AiOutlineMenu } from "react-icons/ai";
import { Line, Pie } from "react-chartjs-2";
import { chart as chartjs } from "chart.js/auto";
import { dataUser } from "./data";
import { calculateNewValue } from "@testing-library/user-event/dist/utils";

const Dashboard = () => {
  const [isOpen, setIsopen] = useState(false);

  const ToggleSidebar = () => {
    isOpen === true ? setIsopen(false) : setIsopen(true);
  };
  const [userData, setUserData] = useState({
    labels: dataUser.map((data) => data.year),
    datasets: [
      {
        label: "user Lost",
        data: dataUser.map((data) => data.Users),
        fill: false,
        backgroundColor: "rgba(75,192,192,0.2)",
        borderColor: "rgba(75,192,192,1)",
        borderWidth: 2,
        lineTension: 0.4,
      },
      {
        label: "user Gain",
        data: dataUser.map((data) => data.guest),
        fill: true,
        borderColor: "#742774",
        borderWidth: 2,
        lineTension: 0.4,
      },
    ],
  });

  const data = {
    labels: ["Red", "Blue", "Yellow"],
    datasets: [
      {
        data: [30, 25, 65],
        backgroundColor: ["red", "blue", "yellow"],
      },
    ],
  };
  const optionsPie = {
    tooltips: {
      callbacks: {
        label: function (tooltipItem, data) {
          // Customize the label here
          return (
            data.labels[tooltipItem.index] +
            ": " +
            data.datasets[0].data[tooltipItem.index]
          );
        },
      },
    },
  };
  const options = {
    scales: {
      y: {
        min: 0, // Set the minimum value for the y-axis
        max: 500, // Set the maximum value for the y-axis
      },

      x: {
        min: 0,

        grid: {
          display: false,
        },
      },
    },
  };

  return (
    <div className="flex  bg-gray-100">
      <div className="w-0 sm:w-1/6">
        <Navbar />
      </div>
      <div className=" bg-gray-100  px-4 sm:w-5/6">
        <div className="head flex justify-between items-center  mt-3">
          <div className="sm:disabled px-2" onClick={ToggleSidebar}>
            <AiOutlineMenu />
          </div>
          <div>
            <h2 className="font-bold text-lg">Dashboard </h2>
          </div>
          <div className="flex  justify-center items-center p-1">
            <div className=" ">
              <input
                type="text"
                className="rounded-xl outline-none p-1"
                placeholder="search..."
              />
            </div>
            <div className="pl-3 pr-3">
              <BiBell className="text-2xl" />
            </div>
            <div className="pl-3 pr-3">
              <img
                src="data:image/jpeg;base64,"
                className="w-9 h-10 rounded-full"
                alt=""
              />
            </div>
          </div>
        </div>

        <div className="grid grid-cols-2  p-2 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <div className="box-border flex flex-col h-28 sm:w-auto  w-auto p-4  rounded-2xl border-gray-400 bg-[#bbf7d0]">
            <div className="  ">
              <FaRegMoneyBillAlt className="" />
            </div>
            <div className="content">
              <p>Total Revenues</p>
              <p>$34243</p>
            </div>
          </div>
          <div className="box-border h-28 w-auto sm:w-auto p-4 rounded-2xl  border-gray-400 bg-[#fecaca]">
            <div className="  ">
              <FaRegMoneyBillAlt className="" />
            </div>
            <div className="content">
              <p>Total Transaction</p>
              <p>1520</p>
            </div>
          </div>
          <div className="box-border h-28 w-auto p-4 sm:w-auto rounded-2xl border-gray-400 bg-[#fbcfe8]">
            <div className="  ">
              <FaRegMoneyBillAlt className="" />
            </div>
            <div className="content">
              <p>Total Likes</p>
              <p>9721</p>
            </div>
          </div>
          <div className="box-border h-28 w-auto sm:w-auto   p-4 rounded-2xl border-gray-400 bg-[#f5d0fe]">
            <div className="  ">
              <FaRegMoneyBillAlt className="" />
            </div>
            <div className="content">
              <p>Total Users</p>
              <p>892</p>
            </div>
          </div>
        </div>
        <div className="box-border  h-64  p-4 mt-4  rounded-2xl border-gray-400 bg-white">
          <div className="search">
            <input type="search" placeholder="search " />
          </div>
          <div className=" border h-44 w-full">
            <Line className="w-full h-44" data={userData} options={options} />
          </div>
        </div>
        <div className="grid grid-cols-1   sm:grid-cols-2 mt-3  gap-4">
          <div className=" flex flex-col h-64 sm:w-auto p-4  rounded-2xl border-gray-400 bg-white">
            <Pie data={data} options={optionsPie} />
          </div>
          <div className="box-border h-64 sm:w-auto p-8 rounded-2xl  border-gray-400 bg-white">
            <div className="flex  justify-between text-center">
              <div className="font-bold text-lg">Today's Schedule</div>
              <div className="seemore">
                <a href="">See All</a>
              </div>
            </div>

            <div className="flex flex-col m-2">
              <div className="mb-2">
                <p>Meeting with supplier from kuta bali</p>
                <p>14:00 - 15:00</p>
                <p>at sunset Road kuta , Bali</p>
              </div>
              <div className="mt-1">
                <p>Check operation at giga factory 1</p>
                <p>18:00 - 20:00</p>
                <p>at Central Jakarta</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Dashboard;
